...
/// Downloads an image in its CDN-provided compressed format so it can be cached efficiently.
pub async fn fetch_image_bytes(client: &Client, url: &str) -> Result<bytes::Bytes> {
    get_bytes(client, url, "Catalog image").await
}
